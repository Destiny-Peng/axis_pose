...
IMAGES_DIR=${1:-}
CAMERA_INFO=${2:-config/d457_color.yaml}
TAG_SIZE=${3:-0.05}
...
python3 tools/extract_groundtruth.py \
  --images "${IMAGES_DIR}" \
...
python3 tools/gt_ingest.py average --input "${OUT_DIR}/groundtruth.csv" --output "${OUT_DIR}/groundtruth_averaged.csv" || { echo "gt_ingest average failed"; exit 3; }

echo "[3/5] Aligning estimates to averaged GT -> ${OUT_DIR}/aligned.csv"
python3 tools/gt_ingest.py align --est "${EST_CSV}" --gt "${OUT_DIR}/groundtruth_averaged.csv" --output "${OUT_DIR}/aligned.csv" --by-sequence || { echo "gt_ingest align failed"; exit 4; }

echo "[4/5] Evaluating aligned pairs -> per-frame and summary in ${OUT_DIR}"
python3 tools/evaluate.py --input "${OUT_DIR}/aligned.csv" --out "${OUT_DIR}/per_frame_errors.csv" --summary "${OUT_DIR}/evaluation_summary.csv" --plots "${OUT_DIR}/plots" || { echo "evaluate failed"; exit 5; }
