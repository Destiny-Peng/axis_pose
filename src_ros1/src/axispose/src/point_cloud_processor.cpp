...
namespace axispose
{
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr PointCloudProcessor::depthMaskToPointCloud(const cv::Mat &depth, const cv::Mat &camera_matrix) const
    {
        ...
        const float constant_x = static_cast<float>(unit_scaling / fx);
        const float constant_y = static_cast<float>(unit_scaling / fy);
        const float center_x = static_cast<float>(cx);
        const float center_y = static_cast<float>(cy);
        ...
        for (int v = 0; v < rows; ++v)
        {
            ...
            {
                const size_t idx = static_cast<size_t>(v) * cols + u;
                ...
                const float z = static_cast<float>(depth_val * unit_scaling);
                const float x = (static_cast<float>(u) - center_x) * static_cast<float>(depth_val) * constant_x;
                const float y = (static_cast<float>(v) - center_y) * static_cast<float>(depth_val) * constant_y;
                                                ...
                                                const PointCloudDenoiseOptions &options) const
    {
        ...
        if (options.use_sor)
        {
            if (cloud->size() >= static_cast<size_t>(std::max(5, options.sor_mean_k)))
            {
        ...
        default:
        {
            ...
            for (size_t i = 0; i < cluster_indices.size(); ++i)
            {
                ...
                if (cluster.indices.size() > largest_cluster_size)
                {
                    largest_cluster_size = cluster.indices.size();
