...
namespace axispose
{
    ...
    // 2D residual: projected 3D line points should lie on fitted mask centerline A*u+B*v+C=0.
    struct ReprojectionLineResidual
    {
        ...
        bool operator()(const T *const plucker, T *residual) const
        {
            ...
            // Closest point p0 on line to origin: p0 = d x m (d is unit).
            T p0[3];
            ...
            // Project using this codebase camera frame: X forward, Y right-neg, Z down-neg.
            ...
            T u = -fx * proj[1] / proj[0] + cx;
            T v = -fy * proj[2] / proj[0] + cy;
    ...
    // Keep optimized line point close to initial axis point to avoid parallel offset drift.
    ...
    // Enforce ||d||=1 and d.m=0 on update.
    struct PluckerPlus
    {
        template <typename T>
        bool operator()(const T *x, const T *delta, T *x_plus_delta) const
        {
            ...
            T dot = ceres::DotProduct(x_plus_delta, x_plus_delta + 3);
                                           ...
                                           double weight_pos)
    {
        ...
        // Build sampled index list to keep optimization bounded.
        ...
        ceres::LocalParameterization *plucker_param =
            new ceres::AutoDiffLocalParameterization<PluckerPlus, 6, 6>();
