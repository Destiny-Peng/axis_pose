...
namespace axispose
{
    ...
    PoseEstimate::PoseEstimate(ros::NodeHandle &nh, ros::NodeHandle &pnh) : nh_(nh), pnh_(pnh)
    {
        ...
        pnh_.param<std::string>("depth_topic", depth_topic, "/camera/depth/image_raw");
        pnh_.param<std::string>("mask_topic", mask_topic, "/yolo/mask");
        pnh_.param<std::string>("camera_info_topic", camera_info_topic, "/camera/camera_info");
        ...
        ROS_INFO("PoseEstimate node starting.");

        ros::NodeHandle nh_local(nh_);
        ros::NodeHandle pnh_local(pnh_);

        message_filters::Subscriber<sensor_msgs::Image> depth_sub(nh_, depth_topic, 5);
        message_filters::Subscriber<sensor_msgs::Image> mask_sub(nh_, mask_topic, 5);
    ...
    void PoseEstimate::denoisePointCloud(pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud)
    {
        ...
        if (!cluster_indices.empty())
        {
            ...
            // auto indices_ptr = pcl::PointIndices<pcl::PointXYZ>::Ptr(cluster_indices[best_idx]);
